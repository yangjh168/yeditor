...
/**
   ...
   */
export function transformIERangeToRange(ieRange, range) {
  if (ieRange.item) {
...
/**
   ...
   */
export function _getIERange(sel) {
  var ieRange;
  //ie下有可能报错
  try {
...
class Selection {
  ...
  rangeInBody(rng, txtRange) {
    ...
    return node === this.document.body || inDoc(node, this.document);
  }
  ...
  getStartElementPath() {
    ...
    if (start) {
      return findParents(start, true, null, true);
    }
    return [];
  }
  ...
  getRange() {
    ...
    if (browser.ie9below) {
      ...
      if (sel && sel.rangeCount) {
        ...
        if (
          this._bakRange &&
          inDoc(this._bakRange.startContainer, this.document)
        ) {
