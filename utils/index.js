...
/**
   ...
   * ```
   */
export function makeInstance(obj) {
...
/**
   ...
   * ```
   */
export function defer(fn, delay, exclusion) {
  var timerID;
  return function() {
    if (exclusion) {
...
/**
   ...
   * ```
   */
export function indexOf(array, item, start) {
  var index = -1;
...
/**
   ...
   * ```
   */
export function unhtml(str, reg) {
  return str
    ? str.replace(
...
/**
   ...
   * ```
   */
export const cssStyleToDomStyle = (function() {
...
/**
   ...
   */
export function fixColor(name, value) {
...
function cssRuleIE (key, style, doc) {
  var indexList, index;
  if (
    style === undefined ||
...
function cssRuleOther(key, style, doc) {
  var head, node;
  if (
    style === undefined ||
    (style && style.nodeType && style.nodeType == 9)
...
/**
   ...
   */
export const cssRule = (browser.ie && browser.version != 11) ? cssRuleIE : cssRuleOther
...
export function serializeParam(json) {
  var strArr = [];
  for (var i in json) {
    //忽略默认的几个参数
...
export function clearEmptyAttrs(obj) {
  for (var p in obj) {
    if (obj[p] === "") {
      delete obj[p];
    }
...
export const json2str = (function() {
  if (window.JSON) {
